...
def create_ppt(title, points, filename="output.pptx", title_size=30, text_size=22,
    ...
    tf.word_wrap, tf.auto_size, tf.vertical_anchor = True, MSO_AUTO_SIZE.TEXT_TO_FIT_SHAPE, MSO_VERTICAL_ANCHOR.MIDDLE
    p = tf.add_paragraph()
...
# ---------------- STATE ----------------
defaults = {"messages": [], "outline_chat": None, "generated_files": [], "summary_text": None, "summary_title": None, "doc_chat_history": []}
...
if uploaded_file:
    with st.spinner("Processing uploaded file..."):
        ...
        if text.strip():
            ...
            title = call_gemini(f"Generate a short title:\n{summary}")
...
if prompt := st.chat_input("💬 Type a message..."):
    if st.session_state.summary_text:
        if any(w in prompt.lower() for w in ["ppt", "slides", "presentation"]):
            st.session_state.outline_chat = {"title": st.session_state.summary_title, "slides": [{"title": "Intro", "description": "• Point 1\n• Point 2"}]}
            ...
            reply = call_gemini(f"Answer using only the doc:\n{st.session_state.summary_text}\n\nQ: {prompt}")
            st.session_state.doc_chat_history.append(("assistant", reply))
    else:
        st.session_state.messages.append(("user", prompt))
        if "ppt" in prompt.lower():
            st.session_state.outline_chat = {"title": "Generated PPT", "slides": [{"title": "Slide 1", "description": "• Point A\n• Point B"}]}
